...
class FullContextStrategy(AnalysisStrategy):
    """一次性全文读取策略
    
    适用场景：
    1. 短论文（<50,000 字符）
    2. 使用大上下文窗口的 LLM（如 128k+ tokens）
    3. 需要完整上下文理解的分析任务
    ...
    def run(self, llm, chunks: List[str], final_prompt: str, max_tokens: int = 4096) -> str:
        ...
        logger.info("执行「一次性全文」策略: 文档长度 %d 字符", len(full_text))
        
        # 直接使用完整文档进行分析
        result = llm._generate(
            final_prompt.format(content=full_text),
...
class StrategyFactory:
    ...
    def get_strategy(strategy_type: Union[ReadStrategy, str]) -> AnalysisStrategy:
        # 统一转换为字符串比较
        s_val = strategy_type.value if isinstance(strategy_type, ReadStrategy) else s_val
        ...
        if s_val == ReadStrategy.FULL_CONTEXT.value:
            return FullContextStrategy()
        
        # 默认使用分层策略
        logger.warning("未知策略类型 '%s'，使用默认分层策略", s_val)
        return HierarchicalMapReduceStrategy()
...
class SectionBasedChunking:
    ...
    def split_by_sections(
        ...
        if not sections:
            logger.warning("未找到任何章节信息，使用固定大小分块")
            return self._split_by_size(text)

        # 优先使用 Level 1 章节（主要章节）
        level1_sections = [s for s in sections if s["level"] == 1]
        level2_sections = [s for s in sections if s["level"] == 2]
        
        logger.info("文档结构: %d 个一级章节, %d 个二级章节", 
                   len(level1_sections), len(level2_sections))
        
        # 选择分块策略
        if level1_sections:
            split_points = sorted([s["start_pos"] for s in level1_sections])
            logger.info("使用 %d 个一级章节作为分块点", len(split_points))
        elif level2_sections:
            # 如果没有一级章节，使用二级章节，但限制数量
            split_points = sorted([s["start_pos"] for s in level2_sections])
            # 如果二级章节太多（>15个），只取前15个
            if len(split_points) > 15:
                logger.warning("二级章节过多 (%d 个)，仅使用前 15 个", len(split_points))
                split_points = split_points[:15]
            logger.info("使用 %d 个二级章节作为分块点（无一级章节）", len(split_points))
        else:
            logger.warning("未找到有效章节，使用固定大小分块")
            return self._split_by_size(text)
    ...
    def _split_by_size(self, text: str, chunk_size: int = 50000) -> List[str]:
        """按固定字符数切分。"""
        chunks = []
        start = 0
