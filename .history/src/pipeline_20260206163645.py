...
class Pipeline:
    ...
    def run(
        ...
        # Convert analysis_type to AnalysisType enum if it's a string
        if isinstance(analysis_type, str):
            try:
                analysis_type = AnalysisType[analysis_type.upper()]
            except KeyError:
                logger.error("无效的分析类型: %s. 使用默认类型: %s", analysis_type, AnalysisType.COMPREHENSIVE.value)
        ...
        if start_stage <= 2:
            ...
            page_contents = ocr_engine.process_all_pages(pages)
        ...
        if chunking.should_chunk(assembly_result.full_markdown, self.config.max_text_length):
            ...
            final_prompt = get_prompt(analysis_type)
