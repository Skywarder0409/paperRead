...
class VisionOCREngine:
    ...
    def process_all_pages(
        ...
        if len(active_ports) < parallel_threads:
            logger.warning("⚠️ 警告: 预设并行 %d 路，但仅检测到 %d 个 Ollama 实例 (%s)", 
                           parallel_threads, len(active_ports), active_ports)
            logger.warning("💡 请确保已执行: OLLAMA_HOST=127.0.0.1:端口 ollama serve")
            if not active_ports:
                raise RuntimeError("未检测到任何可用的 Ollama 服务实例，请先启动服务。")
            # 降级使用仅有的可用端口
            ports = active_ports
            parallel_threads = len(active_ports)

        def _worker_wrapper(img_path, page_num, worker_idx):
            # 将该线程固定分配到一个 Ollama 实例端口
            port = ports[worker_idx % len(ports)]
