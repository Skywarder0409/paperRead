...
class VisionOCREngine:
    ...
    def process_page(
        ...
        try:
            response = client.chat(
                ...
                options={
                    "num_ctx": 20480,  # 提高到 20k，确保复杂页面（大量公式/图表）不被截断
    ...
    def process_all_pages(
        ...
        logger.info("💡 建议: 请确保 Ollama 启动时设置了 OLLAMA_NUM_PARALLEL=%d", parallel_threads)
    ...
    def unload_model(self) -> None:
        """从所有使用的 Ollama 实例中卸载模型以释放显存。"""
        if not self._model_name:
            return

        import ollama
        parallel_count = getattr(self, "_last_parallel_count", 1)
        
        logger.info("🧹 正在从 %d 个实例中清理 OCR 模型显存...", parallel_count)
        ...
        for i in range(parallel_count):
            ...
            try:
                ...
                client.generate(model=self._model_name, keep_alive=0)
                logger.info("   [端口 %d] 模型已卸载", port)
            except Exception:
                # 忽略已经关闭或无法连接的实例
                pass
