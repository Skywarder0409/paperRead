...
class VisionOCREngine:
    ...
    def process_all_pages(
        ...
        with ThreadPoolExecutor(max_workers=parallel_threads) as executor:
            # 建立页面号与任务的映射
            # 我们在 lambda 中包装一下，以便捕获每个任务执行时的线程信息
            future_to_page = {
                executor.submit(self.process_page, str(page.image_path), page.page_num): page.page_num
                for page in pages
            }
            ...
            for future in as_completed(future_to_page):
                ...
                try:
                    ...
                    if show_progress:
                        logger.info(
                            "✅ [%d/%d] 页面 %d 完成 | 线程: %s | 并行总数: %d", 
                            completed_count, total, page_num, thread_name, parallel_threads
                        )
                except Exception as e:
                    logger.error("❌ 页面 %d 处理异常 (线程: %s): %s", page_num, thread_name, e)
