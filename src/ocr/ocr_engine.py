...
_TITLE_EXTRACT_PROMPT = (
    ...
    "{text}\n\n"
    "请从上述内容中找出这篇论文的完整标题。\n"
    "注意：不要把期刊名、作者名、栏目名（如 Note、Article、Letter）当作标题。\n"
    "只返回论文标题本身，不要加任何前缀、解释或引号。"
)


def extract_title_via_llm(model_name: str, first_page_text: str) -> str:
    """通过 LLM 从第一页 OCR 文本中提取论文标题。

    可用任何 Ollama 模型（视觉模型或文本模型均可），只发送文本。
    """
    import ollama

    prompt = _TITLE_EXTRACT_PROMPT.format(text=first_page_text[:3000])
    ...
    try:
        ...
        # 去掉可能的前缀
        for prefix in ["标题：", "标题:", "Title:", "Title：", "论文标题：", "论文标题:"]:
            if title.startswith(prefix):
                title = title[len(prefix):].strip()
        # 去掉可能的换行，只取第一行
        title = title.split("\n")[0].strip()
        logger.info("LLM 提取标题: 「%s」", title)
        return title
    except Exception as e:
        logger.warning("LLM 提取标题失败: %s", e)
        return ""
...
class VisionOCREngine:
    ...
    def process_page(
        ...
        try:
            response = client.chat(
                ...
                options={
                    "num_ctx": 20480,  # 20k上下文：充分利用5090的32GB显存，避免复杂页面截断
    ...
    def process_all_pages(
        ...
        # 并行处理（单实例内部并行，而非多实例物理并行）
        from concurrent.futures import ThreadPoolExecutor, as_completed
        import threading
        logger.info("🚀 启动并行 OCR: 线程数=%d (单实例内部并行)", parallel_threads)
        logger.info("💡 重要: 请手动启动 Ollama 并设置 OLLAMA_NUM_PARALLEL=%d", parallel_threads)
        logger.info("   命令示例: OLLAMA_NUM_PARALLEL=%d ollama serve", parallel_threads)
        
        results = []

        def _worker_wrapper(img_path, page_num):
            # 所有请求都发往默认的 11434 端口（单实例）
            start_time = time.time()
            content = self.process_page(img_path, page_num)
            duration = time.time() - start_time
            t_name = threading.current_thread().name
            return content, t_name, duration

        with ThreadPoolExecutor(max_workers=parallel_threads) as executor:
            # 提交所有任务到线程池
            future_to_page = {
                executor.submit(_worker_wrapper, str(page.image_path), page.page_num): page.page_num
                for page in pages
            }
            ...
            for future in as_completed(future_to_page):
                ...
                try:
                    content, t_name, dur = future.result()
                    results.append(content)
                    completed_count += 1
                    if show_progress:
                        logger.info(
                            "✅ [%d/%d] 页面 %d 完成 | 耗时: %.1fs | 线程: %s | 并发数: %d", 
                            completed_count, total, page_num, dur, t_name, parallel_threads
                        )
                except Exception as e:
                    logger.error("❌ 页面 %d 处理异常: %s", page_num, e)
    ...
    def unload_model(self) -> None:
        """从 Ollama 实例中卸载模型以释放显存。"""
        if not self._model_name:
            return

        import ollama
        
        logger.info("🧹 正在卸载 OCR 模型显存...")
        
        try:
            # 发送 keep_alive: 0 强制立即卸载模型
            ollama.generate(model=self._model_name, keep_alive=0)
            logger.info("✨ OCR 阶段显存已完全释放，Ready for LLM 阶段")
        except Exception as e:
            logger.warning("显存卸载失败（可能服务已停止）: %s", e)
