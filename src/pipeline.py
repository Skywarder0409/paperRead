...
class Pipeline:
    ...
    def run(
        ...
        if start_stage <= 2:
            ...
            # ── 趁视觉模型还在显存中，用第一页 OCR 文本提取论文标题 ──
            # 仅在 PDF 元数据标题不可靠时（像文件名）才用 LLM 覆盖
            if page_contents and metadata:
                import re as _re
                _looks_like_filename = bool(_re.search(
                    r"[-_\.]{2,}|^\d+-\w+-\d+|^1-s2\.", metadata.title
                )) or metadata.title == Path(metadata.file_path).stem
                if _looks_like_filename:
                    from src.ocr.ocr_engine import extract_title_via_llm
                    first_page_text = page_contents[0].markdown
                    llm_title = extract_title_via_llm(self.config.ocr_model, first_page_text)
                    if llm_title and len(llm_title) > 3:
                        logger.info("LLM 标题提取: 「%s」 (替换原标题: 「%s」)", llm_title, metadata.title)
                        metadata.title = llm_title
                else:
                    logger.info("PDF 元数据标题可靠，保留: 「%s」", metadata.title)
        ...
        if use_cache and metadata:
            ...
            if _looks_like_filename:
                from src.ocr.ocr_engine import extract_title_via_llm
                # 从缓存 markdown 中取第一页内容（页面以 --- 分隔）
                first_page = assembly_result.full_markdown.split("---\n<!-- page break -->")[0]
                llm_title = extract_title_via_llm(self.config.llm_model, first_page)
                if llm_title and len(llm_title) > 3:
                    logger.info("缓存模式 LLM 标题提取: 「%s」 (替换: 「%s」)", llm_title, metadata.title)
                    metadata.title = llm_title
        ...
        # 执行策略 (内部会处理单块或多块逻辑)
        analysis_text = chunking.execute_strategy(
            llm, 
            chunks, 
            final_prompt, 
            strategy_type=self.config.read_strategy,
